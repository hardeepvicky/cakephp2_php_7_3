...
class Util
{
    ...
    public static function searilize_array($data)
    {
        if (is_array($data))
        {
            foreach($data as $k => $v)
    ...
    public static function getArrayListFromText($text, $start_char, $end_char)
    {
        $list = array();
        $st_len = strlen($start_char);
        $end_len = strlen($end_char);
        $start_index = false;
        $length = 0;
        $str = "";
        for ($i = 0; $i < strlen($text); $i++)
        {       
            if (substr($text, $i, $st_len) == $start_char)
            {
                $start_index = $i + $st_len;
            }
            
            if ($start_index)
            {                
                if (substr($text, $i, $end_len) == $end_char)
                {                     
                    $end_index = $i - $end_len;
                    $list[] = substr($text, $start_index, $length - 1);
                    $start_index = false;
                    $length = 0;
                }
                else
    ...
    public static function niceDate($days)
    {
        if ($days < 30)
        {
            return $days . " days";
    ...
    public static function unsetKeys($data, $keys)
    {
        foreach($keys as $key)
        {
            unset($data[$key]);            
    ...
    public static function StringOpearion($v, array $operations)
    {
        foreach($operations as $operation)
        {
            switch (strtolower($operation))
            {
                ...
                default :
                    throw new Exception("StringOpearion : $operation not availiable");
            }
        }
